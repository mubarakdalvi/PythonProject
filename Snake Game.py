...
def game_over():
    ...
    canvas.create_text(canvas.winfo_width()/2,canvas.winfo_height()/2,
                       font=('consolas',70),text='GAME OVER',fill='yellow',tags='game_over')
...
canvas = Canvas(window,bg=BACKGORUND_COLOR,height=GAME_HIEGHT,width=GAME_WIDTH)
canvas.pack()

window.update()
window_width = window.winfo_width()
window_hieght = window.winfo_height()
screen_width = window.winfo_screenwidth()
screen_hieght = window.winfo_screenheight()

x = int((screen_width/2) - (window_width/2))
y = int((screen_hieght/2) - (window_hieght/2))

window.geometry(f'{window_width}x{window_hieght}+{x}+{y}')
